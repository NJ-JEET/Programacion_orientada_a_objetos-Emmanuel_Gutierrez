package edu.emmanuel_gutierrez.reto9.process;
...
public class ContadorPalabras {
    ...
    public static List<Map.Entry<String, Integer>> contarPalabras(String nombreArchivo) {
        ...
        try {
            InputStream inputStream = ContadorPalabras.class.getClassLoader().getResourceAsStream("edu/emmanuel_gutierrez/reto9/resources/" + nombreArchivo);
            if (inputStream != null) {
                Scanner fileScanner = new Scanner(inputStream);
                while (fileScanner.hasNext()) {
                    String palabra = fileScanner.next().toLowerCase();
                    palabra = Normalizer.normalize(palabra, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", ""); // Remover acentos
        ...
        return OrganizadorPalabras.organizarPalabras(conteoPalabras);
    }
}
